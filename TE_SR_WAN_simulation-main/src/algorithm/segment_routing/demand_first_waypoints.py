...
class DemandsFirstWaypoints(GenericSR):
    ...
    def __demands_first_waypoints(self):
        ...
        top_k_neighbors = 3  # Adjust this value as necessary

        shortest_paths = {}
        potential_waypoints_dict = {}
        ...
        for d_map_idx in range(len(self.__demands)):
            ...
            if (s, t) not in shortest_paths:
                dijkstra = nk.distance.BidirectionalDijkstra(self.__g, s,t)
                dijkstra.run()
                shortest_paths[(s, t)] = dijkstra.getPath()

            if (s, t) not in potential_waypoints_dict:
                shortest_path_nodes = shortest_paths[(s, t)]
                potential_waypoints = set()
                for node in shortest_path_nodes:
                    neighbors = list(self.__g.iterNeighbors(node))
                    # Get the capacities of the neighbors
                    capacities = [self.__capacity_map[node][neighbor] for neighbor in neighbors]
                    # Combine neighbors and their capacities
                    neighbors_capacities = list(zip(neighbors, capacities))
                    # Sort by capacity
                    neighbors_capacities.sort(key=lambda x: x[1], reverse=True)
                    # Take the top-k neighbors
                    top_k = neighbors_capacities[:top_k_neighbors]
                    # Add to potential waypoints
                    for neighbor, _ in top_k:
                        potential_waypoints.add(neighbor)
                potential_waypoints_dict[(s, t)] = potential_waypoints

            for waypoint in potential_waypoints_dict[(s, t)]:
